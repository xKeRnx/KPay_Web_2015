    ...
    <div class="container youplay-content">
      ...
      <div class="row">
		  ...
		  <?php		
				...
				if(isset($_GET['UID']) AND isset($_GET['RID']) AND $_GET['RID'] != '' AND $_GET['UID'] != ''){
					$UserID = mssql_escape_string($_GET['UID']);
					$RefID = mssql_escape_string($_GET['RID']);
					$checkTransaction = sqlsrv_query($conn, "Select * from tPurchases Where nOwner = ? AND dDate >= Convert(datetime,?,103) AND dDate <= Convert(datetime,?,103) AND nUserID = ? AND ID = ? Order by dDate DESC OFFSET  ? ROWS FETCH NEXT ? ROWS ONLY;", array($_SESSION['nUserID'], $DateFrom, $DateTo, $UserID, $RefID, $OFFSET, $Next), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));							
				}elseif (@$_GET['UID'] == '' AND @$_GET['RID'] == ''){
					$checkTransaction = sqlsrv_query($conn, "Select * from tPurchases Where nOwner = ? AND dDate >= Convert(datetime,?,103) AND dDate <= Convert(datetime,?,103) Order by dDate DESC OFFSET  ? ROWS FETCH NEXT ? ROWS ONLY;", array($_SESSION['nUserID'], $DateFrom, $DateTo, $OFFSET, $Next), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));					
				}elseif(isset($_GET['UID']) AND $_GET['RID'] == ''){
					$UserID = mssql_escape_string($_GET['UID']);
					$checkTransaction = sqlsrv_query($conn, "Select * from tPurchases Where nOwner = ? AND dDate >= Convert(datetime,?,103) AND dDate <= Convert(datetime,?,103) AND nUserID = ? Order by dDate DESC OFFSET  ? ROWS FETCH NEXT ? ROWS ONLY;", array($_SESSION['nUserID'], $DateFrom, $DateTo, $UserID, $OFFSET, $Next), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));				
				}elseif(isset($_GET['RID']) AND $_GET['UID'] == ''){
					$RefID = mssql_escape_string($_GET['RID']);
					$checkTransaction = sqlsrv_query($conn, "Select * from tPurchases Where nOwner = ? AND dDate >= Convert(datetime,?,103) AND dDate <= Convert(datetime,?,103) AND ID = ? Order by dDate DESC OFFSET  ? ROWS FETCH NEXT ? ROWS ONLY;", array($_SESSION['nUserID'], $DateFrom, $DateTo, $RefID, $OFFSET, $Next), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));				
				}else{
					$checkTransaction = sqlsrv_query($conn, "Select * from tPurchases Where nOwner = ? AND dDate >= Convert(datetime,?,103) AND dDate <= Convert(datetime,?,103) Order by dDate DESC OFFSET  ? ROWS FETCH NEXT ? ROWS ONLY;", array($_SESSION['nUserID'], $DateFrom, $DateTo, $OFFSET, $Next), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));				
				}
				
					$RecEx = 0;
					echo '<form id="search" action="https://kernpay.com/user-trans" method="GET">';
					
						echo '<input type="text" placeholder="UserID" value="'.$UserID.'" name="UID">';
						echo '<input type="text" placeholder="REFID" value="'.$RefID.'" name="RID">';
					...
					echo '<thead>';
					 echo ' <tr Style="font-size: 10px;">';
						...
						echo '<th style="width:10%">Ref Id</th>';
						echo '<th style="width:15%" >Project</th>';
						echo '<th style="width:10%">User id</th>';
						echo '<th style="width:15%">Payment method</th>';
						echo '<th style="width:20%">Purchased</th>';
						echo '<th style="width:15%">Datetime</th>';
				...
				while($fetchTransaction = sqlsrv_fetch_array($checkTransaction)){
					$RecEx = 1;
					$SelectProject = sqlsrv_query($conn, "Select ProjectName from tProjects Where ProjectKey = ?;", array($fetchTransaction['sProjectKey']), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));	
					$fetchProject = sqlsrv_fetch_array($SelectProject);
					
					If ($fetchTransaction['sValidate'] == 0){
						$NEWStat = 'Pending';
					}elseif ($fetchTransaction['sValidate'] == 1){
						$NEWStat = 'Finished';
					}else{
						$NEWStat = 'Unknown';
					}
					...
					$DTime = date_format($fetchTransaction['dDate'], 'd-m-Y H:i:s');
					 echo '<tr Style="font-size: 10px;">';
						echo '<th scope="row">'.$i.'</th>';
						echo '<td>'.$fetchTransaction['ID'].'</td>';
						echo '<td>'.$fetchProject['ProjectName'].'</td>';
						echo '<td>'.$fetchTransaction['nUserID'].'</td>';
						echo '<td>'.$fetchTransaction['sStatus'].'</td>';
						echo '<td>'.$fetchTransaction['sDesc'].'</td>';
						echo '<td>'.$DTime.'</td>';
						echo '<td>'.$fetchTransaction['nCountry'].'</td>';
						echo '<td>'.$NEWStat.'</td>';
					echo '</tr>	';				
